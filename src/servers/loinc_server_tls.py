#!/usr/bin/env python3
"""
TLS-secured LOINC TCP Socket Server with Dynamic Batching
Port: 8913
...
class LOINCFAISSTLSServer:
    def __init__(self, host='127.0.0.1', port=8913):  # Loopback only for security
        self.host = host
        self.port = port
    ...
    def _load_resources(self):
        ...
        logging.info("Loading LOINC FAISS index...")
        ...
        logging.info(f"LOINC server ready with {self.index.ntotal} lab tests")
            ...
            if self._should_process_batch():
                ...
                if batch:
                    ...
                    for i, (seq_num, text, response_queue) in enumerate(batch):
                        if indices[i][0] >= 0:
                            loinc_code = self.metadata[indices[i][0]]['loinc_code']
                        else:
                            loinc_code = "NO_MATCH"
                        
                        response_queue.put(f"{seq_num},{loinc_code}\n")
    ...
    def start(self):
        ...
        logging.info(f"TLS-secured LOINC server listening on {self.host}:{self.port}")
        ...
        try:
            ...
            logging.info("Shutting down LOINC TLS server...")
        finally:
            server_socket.close()
