#!/usr/bin/env python3
"""
RxNorm TCP Socket Server with Dynamic Batching
Port: 8904
...
class RxNormFAISSServer:
    def __init__(self, host='127.0.0.1', port=8904):  # Loopback only for security
        self.host = host
        self.port = port
    ...
    def _load_resources(self):
        ...
        print(f"Loaded RxNorm index with {self.index.ntotal} vectors")
        ...
        for i, (seq_num, query, response_queue) in enumerate(batch):
            try:
                ...
                if best_idx >= 0 and best_idx < len(self.metadata):
                    rxnorm_code = self.metadata[best_idx].get('rxcui', 'NO_CODE')
                    score = float(distances[i, 0])
                else:
                    rxnorm_code = 'NO_MATCH'
                    score = 0.0
                
                response_queue.put((seq_num, rxnorm_code, score))
            ...
            while True:
                ...
                for line in lines:
                    ...
                    try:
                        ...
                        # Wait for response
                        try:
                            seq_num_resp, rxnorm_code, score = response_queue.get(timeout=5.0)
                            response = f"{seq_num_resp},{rxnorm_code}\n"
    ...
    def start(self):
        ...
        print(f"RxNorm TCP Server listening on {self.host}:{self.port}")
        print("Protocol: sequence_number,description -> sequence_number,rxnorm_code")
        print(f"Using device: {self.device}")
