...
Based on the working non-TLS server with added TLS support
"""
...
def inference_worker():
    ...
    while running:
        try:
            ...
            # Update stats
            with stats_lock:
                stats['total_requests'] += response_count
                stats['total_batches'] += 1
                stats['total_inference_time'] += inference_time
                stats['total_time'] += total_time
                
            logger.debug(f"Batch processed: size={batch_size}, "
                        f"wait={avg_wait_time*1000:.1f}ms, "
                        f"inference={inference_time*1000:.1f}ms, "
                        f"total={total_time*1000:.1f}ms, "
                        f"throughput={batch_throughput:.1f} req/s")
        ...
        while running:
            try:
                ...
                for request in requests:
                    ...
                    # Add to batcher
                    req = Request(
                        id=f"{client_address[0]}:{client_address[1]}:{request_count}",
                        seq_num=seq_num,
                        text=text.strip(),
...
def stats_reporter():
    ...
    while running:
        ...
        # Calculate rates
        requests_diff = current_stats.get('total_requests', 0) - last_stats.get('total_requests', 0)
        time_diff = 10.0  # seconds
        
        if requests_diff > 0:
            qps = requests_diff / time_diff
            
            # Average batch size
            batches_diff = current_stats.get('total_batches', 0) - last_stats.get('total_batches', 0)
            avg_batch_size = requests_diff / batches_diff if batches_diff > 0 else 0
            
            logger.info(f"Stats: {qps:.1f} QPS, "
                       f"avg batch size: {avg_batch_size:.1f}, "
                       f"total requests: {current_stats.get('total_requests', 0)}")
        
        last_stats = current_stats

def start_server():
    """Start the TLS-secured dynamic batching server"""
    ...
    logger.info(f"TLS-secured dynamic batching server listening on {HOST}:{PORT}")
