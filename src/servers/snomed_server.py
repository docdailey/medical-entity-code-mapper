#!/usr/bin/env python3
"""
TCP Server for SNOMED CT code assignment using FAISS similarity search
Port: 8902
...
class SNOMEDFAISSServer:
    def __init__(self, host='127.0.0.1', port=8902):  # Loopback only for security
        self.host = host
        self.port = port
        self.running = False
        ...
        logging.info(f"SNOMED server initialized with {len(self.metadata)} entries")
            ...
            try:
                ...
                for i, (query_embedding, request) in enumerate(zip(query_embeddings, batch)):
                    ...
                    if indices[0][0] < len(self.metadata):
                        ...
                        snomed_code = best_match.get('snomed_code', best_match.get('code', 'UNKNOWN'))
                    ...
                    # Send response
                    request['callback'](snomed_code, confidence)
    ...
    def handle_client(self, client_socket, address):
        ...
        try:
            while self.running:
                ...
                def callback(snomed_code, confidence):
                    response_data['code'] = snomed_code
    ...
    def start(self):
        ...
        logging.info(f"SNOMED server listening on {self.host}:{self.port}")
    ...
    def stop(self):
        ...
        logging.info("SNOMED server stopped")
