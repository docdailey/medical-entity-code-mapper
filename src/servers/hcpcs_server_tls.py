#!/usr/bin/env python3
"""
TLS-secured TCP Server for HCPCS code assignment using FAISS similarity search
Port: 8915
...
class HCPCSFAISSTLSServer:
    def __init__(self, host='127.0.0.1', port=8915):  # Loopback only for security
        ...
        logging.info(f"HCPCS server ready with {self.index.ntotal} items")
    ...
    def process_batch(self, batch):
        ...
        for i, (seq_num, text, client_socket) in enumerate(batch):
            try:
                if indices[i][0] >= 0:
                    hcpcs_code = self.metadata[indices[i][0]]['hcpcs_code']
                else:
                    hcpcs_code = "NO_MATCH"
                
                response = f"{seq_num},{hcpcs_code}\n"
    ...
    def start(self):
        ...
        logging.info(f"TLS-secured HCPCS server listening on {self.host}:{self.port}")
    ...
    def stop(self):
        """Stop the server"""
        self.running = False
        logging.info("HCPCS TLS server stopped")
